...
@section('title', 'Edit Medical Alter - Dashboard Admin')
@section('page-title', 'Edit Medical Alter')
@section('page-description', 'Update alternative medical treatment information')
...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('dashboardadmin.services.medicalalter.update', $medicalalter->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-yellow-50">
                <h2 class="text-xl font-semibold text-gray-900">
                    <i class="fas fa-pills mr-2 text-yellow-600"></i>Edit Medical Alter: {{ $medicalalter->name }}
                </h2>
                <p class="text-sm text-gray-600 mt-1">Update the information below to modify this alternative medical treatment</p>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $medicalalter->name) }}" 
                    ...
                    <div>
                        ...
                        <select name="type" id="type" 
                            ...
                            <option value="Herbal Medicine" {{ old('type', $medicalalter->type) == 'Herbal Medicine' ? 'selected' : '' }}>Herbal Medicine</option>
                            <option value="Acupuncture" {{ old('type', $medicalalter->type) == 'Acupuncture' ? 'selected' : '' }}>Acupuncture</option>
                            <option value="Massage Therapy" {{ old('type', $medicalalter->type) == 'Massage Therapy' ? 'selected' : '' }}>Massage Therapy</option>
                            <option value="Aromatherapy" {{ old('type', $medicalalter->type) == 'Aromatherapy' ? 'selected' : '' }}>Aromatherapy</option>
                            <option value="Homeopathy" {{ old('type', $medicalalter->type) == 'Homeopathy' ? 'selected' : '' }}>Homeopathy</option>
                            <option value="Chiropractic" {{ old('type', $medicalalter->type) == 'Chiropractic' ? 'selected' : '' }}>Chiropractic</option>
                            <option value="Other" {{ old('type', $medicalalter->type) == 'Other' ? 'selected' : '' }}>Other</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="intro" id="intro" rows="3" 
                              ...
                              placeholder="Brief introduction about this medical alter">{{ old('intro', $medicalalter->intro) }}</textarea>
                    @error('intro')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="benefits" id="benefits" rows="4" 
                              ...
                              placeholder="List the benefits of this alternative treatment">{{ old('benefits', $medicalalter->benefits) }}</textarea>
                    @error('benefits')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" 
                              ...
                              placeholder="Detailed description of the medical alter">{{ old('description', $medicalalter->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-yellow-600 hover:text-yellow-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-yellow-500">
                                    <span>{{ $medicalalter->image ? 'Change image' : 'Upload a file' }}</span>
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $medicalalter->meta_title) }}" 
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" id="meta_description" rows="3" 
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                                      placeholder="SEO meta description">{{ old('meta_description', $medicalalter->meta_description) }}</textarea>
                        </div>

                        <div>
                            <label for="meta_keywords" class="block text-sm font-medium text-gray-700 mb-2">Meta Keywords</label>
                            <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $medicalalter->meta_keywords) }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                ...
                <div class="border-t border-gray-200 pt-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="status" id="status" value="1" {{ old('status', $medicalalter->status) ? 'checked' : '' }}
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Update Medical Alter
                </button>
            </div>
        </div>
    </form>
