...
@section('title', 'Edit Destination - Medical Services')
@section('page-title', 'Edit Destination')
@section('page-description', 'Update destination information')
    ...
    <div class="max-w-4xl mx-auto">
        <div class="form-card rounded-2xl shadow-2xl p-8">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-gray-600">Update destination information and settings</p>
            </div>

            <form action="{{ route('dashboardadmin.management.destination.update', $destination) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="6" required
                                      class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="Describe the destination and its services">{{ old('description', $destination->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="url" id="map_url" name="map_url" value="{{ old('map_url', $destination->map_url) }}"
                                   class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $destination->sort_order) }}" min="0"
                                       class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                @error('sort_order')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class="fas fa-image mr-2 text-blue-600"></i>Current Image
                            </label>
                            <div class="relative">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class="fas fa-upload mr-2 text-blue-600"></i>{{ $destination->hasImage() ? 'Replace Image' : 'Upload Image' }}
                            </label>
                            <div class="drag-drop-area p-6 text-center" id="dropArea">
                                ...
                                <div id="imagePreview" class="hidden">
                                    ...
                                    <button type="button" onclick="removeImage()" 
                                        ...
                                        <i class="fas fa-trash mr-2"></i>Remove New Image
                                    </button>
                                </div>
                            </div>
                            @error('image')
                        ...
                        <div class="bg-gray-50 rounded-lg p-6">
                            ...
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <div id="previewCardImage" class="h-32 bg-gray-200 flex items-center justify-center">
                                    @if($destination->hasImage())
                                        <img src="{{ $destination->image_url }}" alt="Preview" class="w-full h-full object-cover">
                                    @else
                                ...
                                <div class="p-4">
                                    ...
                                    <p id="previewDescription" class="text-sm text-gray-600">{{ Str::limit($destination->description, 100) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    ...
                    <div class="flex space-x-4">
                        ...
                        <button type="submit" 
                                class="bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white px-8 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl flex items-center">
...
@push('scripts')
<script>
// Image upload and preview (same as create page)
...
function removeImage() {
    ...
    @if($destination->hasImage())
        previewCardImage.innerHTML = '<img src="{{ $destination->image_url }}" alt="Preview" class="w-full h-full object-cover">';
    @else
